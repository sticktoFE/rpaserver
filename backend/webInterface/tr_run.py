...
from backend.tools.model import OcrHandle
from json import dumps
from os import path
from backend.tools.np_encoder import NpEncoder
from backend.tools import log
from backend.config import max_post_time ,dbnet_max_size,white_ips
import logging
...
request_time = {}
now_time = strftime("%Y-%m-%d", localtime(time()))



class TrRun(RequestHandler):
    '''
    使用 tr 的 run 方法
    '''
    def get(self):
    ...
    def post(self):
        ...
        if img_up is not None and len(img_up) > 0:
            ...
            raw_image = b64decode(img_b64.encode('utf8'))
            img = Image.open(BytesIO(raw_image))
        else:
            self.set_status(400)
            logger.error(dumps({'code': 400, 'msg': '没有传入参数'}, cls=NpEncoder))
            self.finish(dumps({'code': 400, 'msg': '没有传入参数'}, cls=NpEncoder))
            return
        ...
        try:
            ...
            error_log = dumps({'code': 400, 'msg': '产生了一点错误，请检查日志', 'err': str(ex)}, cls=NpEncoder)
            logger.error(error_log, exc_info=True)
            self.finish(error_log)
            return
        img = img.convert("RGB")
        
        time_now = strftime("%Y-%m-%d-%H_%M_%S",localtime(time()))
        time_day = strftime("%Y-%m-%d", localtime(time()))
        ...
        if do_det:
            ...
            for i, r in enumerate(res):
                ...
                myfont = ImageFont.truetype(path.join(currentdir,"仿宋_GB2312.ttf"), size=size)
            ...
            img_detected_b64 = b64encode(byte_data).decode('utf8')
        ...
        logger.info(dumps(log_info, cls=NpEncoder))
        self.finish(dumps(
            {'code': 200, 'msg': '成功',
             'data': {'img_detected': 'data:image/jpeg;base64,' + img_detected_b64, 'raw_out': res,
                      'speed_time': round(time() - start_time, 2)}},
            cls=NpEncoder))
        return
